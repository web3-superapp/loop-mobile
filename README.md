...
- 通知导航的纯 Dart 基础已完成：只接受严格、版本化、绑定后端 Stream 身份的 LOOP 自有 `notification.v1` 数据，前台/后台到达不会自动跳页，用户点击也只能落到官方 Chat CID、Audio Room 大厅或通知中心。生产通知页不再展示伪实时卡片，演示卡仅在显式 `开发预览` 中可见
