...
- D0 公共 metadata 客户端已严格接入 `GET /v2/meta/client-policy` 与 `GET /v2/meta/capabilities`：`LoopApp` 根生命周期在有效 Development 配置下冷启动一次非阻塞并发观察，请求不携带 Bearer、body、query、`X-Loop-*` 或幂等头，响应必须满足 V2 七字段错误、`no-store` 与请求 ID 证明。错误只停留在 Riverpod observation，不阻断登录或路由；`unavailable`、`deferred` 和 `evidence.pending` 都不等于允许、已连接或已通过真机验收，也不会直接触发强更、地区或维护页面。D0 不提供 maintenance notice。决策 0064 在不改变以上任何一条的前提下补上失败重取：`LoopV2MetaObserver` 冷启动取一次，失败后最多退避重试 5 次（1s→2s→4s→8s→16s），并在网络恢复（`connectivity_plus`）、App 回到前台、以及每次路由跳转（即任意模块页在能力清单缺席时打开）各触发一次；所有触发单飞去重，读取进行中或已排定退避重试时忽略，已完成的观察（含未配置后端时的 `null`）永不重取，只有 `AsyncError` 会被重取。成功后各模块门自然打开，无需重启
- 会话恢复是三态：`authenticated`/`authenticatedUnverified`、`signedOut`、以及决策 0064 新增的 `restoreUnavailable`。只有 Privy 明确返回 `Unauthenticated`（来自 `getAuthState` 或 `authStateStream`）才会显示登录表单；网络类失败与无法归类的失败都停在品牌启动帧上的「暂时无法确认登录状态」+ 原因 + 单飞「重试」，不声称用户被登出。privy_flutter 0.10.1 的 `PrivyException`只带 `message`（`convertToPrivyException` 仅为四个 MFA code 保留 `PlatformException.code`），因此 `PrivyFailureClassifier` 按报文匹配：先判传输类标记，再判凭据类标记，其余归为 `unknown` 并与传输类同等对待
...
- 第 8 步安全/设置/客服已接 V2：撤销他人设备是审计投影（LOOP 拒绝该 `X-Loop-Session-ID` 的后续请求并写一条 `security.event`，对方 Privy 访问未终止），页面只说"已记录撤销，对方访问未终止"；撤销本机与下线全部设备恒为需要二次验证；六项 Privy 安全能力全部不可用并各自给出原因，不渲染安全评分、已开启徽章、遮罩私钥、守护人名单、设备名称或地理位置；账号设置为服务端固定值只读，减少动效与主题留在本机；`about` 由服务端下发版本、规则快照与无版本号的开源清单，版本与构建号本地读取；`pay`/`bridge`/`bridge-status`/`dapp` 只保留入口并展示服务端 reasonCode，DApp 网址核对完全在本机完成、不发起任何请求、不跟随跳转，连接与签名保持禁用
