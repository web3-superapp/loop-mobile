...
- Chat 顶部现提供固定的 `创建群组` / `添加好友` 菜单，Profile 的 People & communication 增加 `我的好友`。主昵称发现、待确认的外发好友请求、好友列表与选择 2–29 位好友建群已经完成前端模型、状态和路由；生产默认好友端口在后端社交契约到达前保持不可用，不发起社交后端请求或建群/成员写入。只有显式 Development Preview 注入进程内好友目录，并持续标注 `开发预览`；预览建群不返回 Stream CID、不创建频道。主昵称、群内匿名昵称、钱包地址、Privy/LOOP/Stream 身份与 typed viewer-scoped 好友引用保持分离；重复昵称在公开区分字段完成前失败关闭，模糊写入在当前账号会话内保留 UUID 与草稿并冻结。生产建群只有拿到 canonical Chat CID 后才报告成功，CID 路由还会用精确 CID + 当前成员条件向 Stream 查询已存在频道，未确认 membership 时不会挂载频道 UI；该查询仍属于官方 Chat 读取/监听链路，并非“零 Stream 请求”。分页、收到请求后的接受/拒绝和超时对账仍等待后端契约
