...
"""
    ...
    def test_primary_feature_cannot_mount_perp_route(self) -> None:
        # Step 5 deleted the Home slice, so the guard is exercised through
        # another mounted primary destination.
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            path = root / "lib/features/market/market_screen.dart"
            path.parent.mkdir(parents=True)
            path.write_text("context.push('/perp/account');\n", encoding="utf-8")
            result = check_harness.check_spot_only_product_contract(root)

        self.assertIn(
            "lib/features/market/market_screen.dart must not mount a retained Perp product route",
            result,
        )
    ...
    def test_retired_home_slice_cannot_be_restored(self) -> None:
        # Step 5 deleted `lib/features/home/home_screens.dart` (decision 0057).
        # Home, a standalone notification centre and the Home security page are
        # product red lines, so the file returning at all is the finding.
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            path = root / "lib/features/home/home_screens.dart"
            path.parent.mkdir(parents=True)
            path.write_text("class HomeScreen {}\n", encoding="utf-8")
            result = check_harness.check_spot_only_product_contract(root)

        self.assertTrue(
            any(
                "is retired (decision 0057) and must not be restored" in error
                for error in result
            ),
            msg=f"expected retired Home slice guard: {result}",
        )
    ...
    def test_only_one_factory_can_mint_a_backend_canonical_intent(self) -> None:
        relative = "lib/core/intent/signing_intent.dart"
        source = (REPOSITORY_ROOT / relative).read_text(encoding="utf-8")
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
        ...
        self.assertTrue(
            any(
                "IntentOrigin.backendCanonical" in error for error in result
            ),
            msg=f"expected single canonical-origin guard: {result}",
        )

    def test_unsigned_transaction_keys_cannot_drift(self) -> None:
        relative = "lib/features/wallet/money_actions_models.dart"
        source = (REPOSITORY_ROOT / relative).read_text(encoding="utf-8")
        mutations = (
            ("    'gasPrice': gasPrice,\n", ""),
            ("'nonce': nonce,", "'transactionNonce': nonce,"),
        )
        for original, replacement in mutations:
            with self.subTest(mutation=original.strip()):
                with tempfile.TemporaryDirectory() as temporary:
                ...
                self.assertTrue(
                    any(
                        "LoopUnsignedTransaction.toWire must emit exactly" in error
                        for error in result
                    ),
                    msg=f"expected verbatim transaction guard: {result}",
                )

    def test_money_action_pages_cannot_take_a_transport_dependency(self) -> None:
        ...
        self.assertTrue(
            any("must stay a port consumer" in error for error in result),
            msg=f"expected money-action transport guard: {result}",
        )

    def test_swap_confirmation_cannot_drop_the_device_evidence_gate(self) -> None:
        relative = "lib/features/wallet/swap_screens.dart"
        source = (REPOSITORY_ROOT / relative).read_text(encoding="utf-8")
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            target = root / relative
            target.parent.mkdir(parents=True)
            target.write_text(
                source.replace("capability.isUsable", "capability.isAvailable"),
                encoding="utf-8",
            )
            result = check_harness.check_s6_money_action_contract(root)

        self.assertTrue(
            any("capability.isUsable" in error for error in result),
            msg=f"expected Swap evidence gate guard: {result}",
        )

    def test_signing_exit_cannot_skip_its_two_admission_checks(self) -> None:
        ...
        for original in mutations:
            with self.subTest(mutation=original):
                with tempfile.TemporaryDirectory() as temporary:
                    ...
                    self.assertIn(original, source)
                    target.write_text(
                        source.replace(original, "if (false)", 1), encoding="utf-8"
                    )
                    result = check_harness.check_s6_money_action_contract(root)

                self.assertTrue(
                    any(original in error for error in result),
    ...
    def test_push_registration_contract_requires_the_community_arrival_gate(
        ...
        source = (check_harness.ROOT / coordinator_relative).read_text(
            encoding="utf-8"
        )
        self.assertEqual(source.count("if (!_readCommunityReached()) {"), 2)
        for count in (0, 1):
            with self.subTest(remaining=count):
                with tempfile.TemporaryDirectory() as temporary:
                    root = Path(temporary)
                    coordinator = root / coordinator_relative
                    ...
                    result = check_harness.check_push_registration_contract(
                        root
                    )
    ...
    def test_push_registration_contract_requires_the_landing_arrival(
        ...
        mutations = (
            # The arrival stops being marked at all.
            (
                "if (next.landing == LoopProfileLanding.community) "
                "_onCommunityArrival();\n",
                "",
            ),
            # The end of the opening stops marking it.
            ("          _onCommunityArrival();\n", ""),
            # Or it goes back to reading a route location, which a deep link
            # under `/community/…` never matches.
            (
                "if (next.landing == LoopProfileLanding.community) "
                "_onCommunityArrival();",
    ...
    def test_push_registration_contract_refuses_a_repeated_prompt(
        ...
        self.assertTrue(
            any(
                "read a refusal rather than repeat it" in error
                for error in result
            ),
            msg=f"expected repeated-prompt guard: {result}",
        )
