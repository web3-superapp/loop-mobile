...
- The current `notification.v1` router is a provider-neutral classifier only. It accepts exact LOOP-owned String envelopes, binds interactions to the backend-derived Stream user ID, and produces only official Chat CID, Audio Room lobby, notification-center, or price-alert token intents. The price-alert kind (decision 0055) validates its `asset_id` against the canonical CAIP pattern before it can become a location. The router is not a raw Firebase/Stream payload parser and does not prove delivery.
