...
8. Preserve one centralized Firebase Messaging foreground/background router. Chat and Video must not register competing global background handlers. Device registration has one owner too: `LoopPushRegistrationCoordinator` asks the OS for permission only after a backend-verified account exists, reports the token to `POST /v2/devices/push-token` and to Stream (`firebase` on Android, `LOOPAPNS` with the APNs device token on iOS), and revokes all three before sign-out tears the session down.
