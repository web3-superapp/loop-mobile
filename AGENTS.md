...
8. Preserve one centralized Firebase Messaging foreground/background router. Chat and Video must not register competing global background handlers. Device registration has one owner too: `LoopPushRegistrationCoordinator` waits for `pushNotifications` to read available, asks the OS for permission once per account only after a backend-verified session exists, and reports the **Firebase** registration token to `POST /v2/devices/push-token` on both platforms. Stream is a second sender with its own addressing: `firebase` on Android, `LOOPAPNS` with the APNs device token on iOS. A `503 PUSH_RUNTIME_DEFERRED` stops the attempt for the run instead of retrying.
...
15. Keep Watchlist models limited to versioned, grouped, ordered canonical CAIP `assetId` references under a whole-resource version CAS. The Watchlist is not a market fact: it never carries a price, a change, freshness, tradability or alert state. Its Preview adapter was retired with decision 0057; the surface is unavailable outside the authenticated V2 adapter. Decision 0066 makes `token`'s star the only path that adds an asset: it toggles membership with one whole-document `PUT` under `expectedVersion`, writing into the client-named `default` group and creating that group in the same replacement. The server generates no group key and rejects `Idempotency-Key` on this write, so the key is generated on the client and no idempotency key is issued. A `VERSION_CONFLICT` is re-read and retried exactly once, never in a loop. `watchlist-edit` orders, removes and creates groups; it never adds an asset.
...
18. Keep Notification Preferences limited to the contract's exact ten categories under a version CAS. `security.event` is locked on and every write submits `true`, because the server rejects `false` rather than ignoring it. The channel now has two states: while `push` is unavailable the page passes on the server's own reason, and once it is available the page says only that no device has been seen receiving anything yet. The UI must never infer Firebase, provider, device permission or delivery from a stored intent — a registered device token says where a message could go, never that one was sent.
...
29. Keep every S6 money action server-owned. The client never builds call data, a fee, a nonce or a signature payload: it renders the server's intent, hands the payload to the wallet unchanged, and reports back what the wallet produced. `SigningIntent.backendCanonical` is the only constructor that may reach a wallet — it carries the verbatim payload and the server's `reviewSha256`, so the facts on screen and the bytes that get signed are two halves of one canonical object, and a locally assembled preview intent is refused at the wallet boundary itself. `signing.allowed` is never derived locally; only expiry is evaluated on device, and it disables the confirmation. A wallet result is not an outcome: a hash or a signature goes straight to `broadcast-report` / `execute`, and only `tx-result` may announce success, on `confirmed` alone. `unknown` is a locked state that is polled and never resubmitted, and an unresolved report locks rather than inviting a second signature. The Swap confirmation stays closed while `privySwap.evidence` is pending; unlimited approval requires the guard's own warning and the server's `acknowledgeUnlimited`; recipient screening, approval risk facts, the platform fee and the slippage policy stay explicit gaps with their reason codes.
30. Keep the D20 security surfaces limited to what the two server projections state. A revoke is an audit projection: LOOP stops accepting the revoked `X-Loop-Session-ID` and writes one `security.event`, while the other device keeps its provider access, so no surface may say a device was signed out. The six Privy security methods stay unavailable with their own reason; no protection score, enabled badge, masked private key, guardian list, device name or device location may be rendered. Account settings are fixed server constants shown read-only, `reduceMotion` and `theme` stay device-local, and `about` reads the client build from the device while the server publishes only versions. `pay`, `bridge`, `bridge-status` and `dapp` are entry points that carry the server's deferred reason and hold no amount, route, fee or progress source; the DApp review is a pure local function that never opens, resolves or follows the address it inspects, and connect, sign and domain reputation stay unavailable.
...
- A Stream API key alone is not an authenticated connection. Do not construct Video or connect Chat until a validated backend bootstrap returns the server-derived Stream user ID and the strict backend token loader succeeds. The Chat/Video token clients are implemented, but live provider/device behavior remains unverified and Audio Room still has no production room locator. Firebase is initialized (S70) from `lib/firebase_options.dart` and the two committed console files, but only for a build whose `FIREBASE_CONFIGURED` and build profile agree; delivery on a physical device is still unverified.
...
- The notification router classifies decision 0067's four-key payload — `type`, `entityRef`, `contextRoute`, `eventVersion: "1"` — and nothing else. It produces a pointer, never a location: the destination comes from re-reading `GET /v2/notifications/feed` for the account signed in now, and a price alert whose asset that feed does not confirm opens the alerts page rather than a token chosen from the payload. A fifth key, a mismatched `type`/`contextRoute` pair, or another sender's payload (Stream's own chat push included) fails closed. The router is not a raw Firebase/Stream payload parser and does not prove delivery.
...
- `lib/integrations/notifications/firebase_notification_ingress.dart` is the only location allowed to own Firebase global callbacks and the only one allowed to call `Firebase.initializeApp`. It normalizes `RemoteMessage.data` and nothing else: no background handler, no foreground banner, no provider display copy, no PushKit. A `RemoteMessage` never leaves that file.
...
- `loopNotificationEventSourceProvider` and `loopPushTokenSourceProvider` both default to their disabled implementations. Only `lib/main.dart` overrides them, exactly once, and only after `config.canInitializeFirebase` and a successful `LoopFirebaseIngress.ensureApp()`. Tests and the offline Preview entry point therefore still reach no provider.
