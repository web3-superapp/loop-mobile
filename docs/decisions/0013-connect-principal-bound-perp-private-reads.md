...
- Bind wallet creation to an explicit expected Privy principal. The SDK
  single-flight may be shared only by that same principal, and its result
  carries the creating principal so the session controller rejects a late or
  accidentally reused result before attaching any wallet address.
...
- Keep `LOOP_BACKEND_BASE_URL` an explicit client-safe build input. Physical
  ...
  no private request.
- Declare Android `INTERNET` in the main manifest so the same HTTPS-only
  production adapters work in Release. This normal permission does not enable
  Firebase, push, background execution, or any optional Stream call service.
