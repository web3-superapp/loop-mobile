...
- Treat candle open/close time as provider row data and record client receipt time separately before parsing. The last returned candle may still be forming; label that state and warn that refresh can change its OHLCV. Loading, empty, error, refresh, and retry remain explicit, while Preview candles, another asset, Perp data, gap filling, and executable actions are forbidden fallbacks.
