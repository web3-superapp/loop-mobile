...
- The D8 Perp account slice establishes the production private-read adapter: a verified Privy principal and locally observed wallet rotate one in-memory backend session; the backend alone derives wallet authority. Explicit binding is versioned and reconciled without replay, while config/account facts expire at the earliest backend source deadline. D4 now mounts short-lived backend positions with a bounded two-item initial page, cursor-only continuation, earliest-page expiry, Decimal rendering, and no automatic binding or management route. D5 production detail fails closed; orders, fills, and funding transports remain unmounted. Trading, signing, close/reduce, leverage, transfer, withdrawal, and every other unconnected production port remain unavailable. Feature modules still own no Dio or backend route literals, and `main_preview.dart` remains the only labelled fixture composition.
