...
- The retained Hyperliquid Core perpetual implementation is disabled history and must not be relabelled as spot. Decision 0017 accepts a separate public Hyperliquid Testnet spot market-data contract; spot execution remains unavailable and requires its own backend-mediated decision.
...
- Decisions 0013 and 0014 remain implementation history, while decision 0016 keeps their Perp account and position surfaces unmounted from product navigation. Their principal-bound, strict-expiry and Decimal-safe code remains disabled and receives no further feature work. Decision 0017 connects Market only to Hyperliquid Testnet's public `spotMetaAndAssetCtxs` info response through a separate read-only adapter: tokens are joined by sparse token index, asset contexts are joined to universe rows by exact provider `coin`, and every price and volume remains an exact wire String plus `Decimal`. The response has no exchange snapshot timestamp, so the UI identifies only the UTC client receipt time. The default ledger filters out zero-24h-volume pairs, sorts by exact 24h notional volume, and caps display at 50; search may match all returned pairs but remains capped at 50. Spot account, balance, order, cancellation, execution, signing, transfer, withdrawal, and every other private path remain unavailable and backend-owned. Feature modules still own no Dio or backend route literals, and `main_preview.dart` remains the only labelled fixture composition.
