...
- The D8 Perp account slice is the first production private-read adapter: a verified Privy principal and locally observed wallet rotate one in-memory backend session; the backend alone derives wallet authority. Explicit binding is versioned and reconciled without replay, while config/account facts expire at the earliest backend source deadline. Positions, orders, fills, and funding transports are ready but have no production projection yet. Trading, signing, leverage, transfer, withdrawal, and every other unconnected production port remain unavailable. Feature modules still own no Dio or backend route literals, and `main_preview.dart` remains the only labelled fixture composition.
